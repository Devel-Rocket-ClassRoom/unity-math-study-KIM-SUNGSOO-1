...
public class LerpBasics : MonoBehaviour
{
    ...
    private void UpdateHPLerp()
    {
        if(useAbsoluteLerp)
        {
            ...
            float t = Mathf.Clamp01(lerpElapsedTime);
    ...
    private void UpdateColorLerp()
    {
        ...
        float color1 = Mathf.PingPong(colorLerpTime * colorLerpSpeed, 1f);

        materialInstance.color = Color.Lerp(colorStart, colorEnd, color1);
        debugColorT = color1;
    }
