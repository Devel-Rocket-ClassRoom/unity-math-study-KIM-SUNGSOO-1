...
public class Assignment_CircularFormation : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < units.Length; i++)
        {
            ...
            Quaternion finalRotationQuat = leader.rotation * rotationQuat;
            Vector3 dir = finalRotationQuat * Vector3.forward;
           
            units[i].transform.position = leader.transform.position + (dir * formationRadius);
            Vector3 toTarget = (units[i].position - transform.position).normalized;
            units[i].rotation = Quaternion.LookRotation(toTarget, Vector3.up);
