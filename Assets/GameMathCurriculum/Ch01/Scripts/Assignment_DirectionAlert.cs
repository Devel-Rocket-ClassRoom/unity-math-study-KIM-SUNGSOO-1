...
public class Assignment_DirectionAlert : MonoBehaviour
{
    ...
    private Direction GetDirection(Transform enemy)
    {
        ...
        float angle = Vector3.Dot(transform.forward, targetDirection.normalized);
        ...
        if (distance < alertRange && angle <= 45f && angle != 1)
        {
            return Direction.Front;
        }
        else if (distance < alertRange && angle  >= 135f && angle != -1)
        {
            return Direction.Back;
        }
        else if (distance < alertRange && cross.y <= -0.1f)
        {
